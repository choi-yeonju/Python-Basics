...
def report_fitness(fit):
    ...
    summary() 결과표의 문자열을 파싱하지 않고, 
    'fit' 객체가 이미 갖고 있는 속성에서 지표를 직접 읽어와 문장을 구성한다. 
    ...
    Returns:
        str: 모형 적합도 보고 문장. 
        'IPython.display.Markdown'으로 감싸 출력하면 좋다. 
    """
    # --- 1) 변수 라벨 구성 (상수항 제외) ---
    yname = fit.model.endog_names
...
def report_variables(fit, data):
    ...
    Returns:
        DataFrame : 독립변수별 보고표. 종속변수, 독립변수, 
                    B, 표준오차, z, 유의확률, 오즈비(OR), OR 95% 신뢰구간, 
                    공차, VIF 컬럼을 가진다.
    ...
    for x in xnames:
        ...
        # 표준화 회귀계수
        # --> (베타std) = B x 독립변수 표준편차 = "독립변수  1 SD 변화당 log(오즈비) 변화"
        # 로지스틱은 종속변수가 0/1 이라 (OLS와 달리) y로는 표준화하지 않고 독립변수만 표준화한다 
...
def report_variables_text(fit, data=None, alpha=0.05):
    ...
    각 독립변수에 대해 계수(B), 오즈비(OR), z, 유의확률을 문장으로 풀어쓰고,
    오즈비를 백분율 변화로 환산하여 "오즈가 약 몇 % 증가/감소" 형태로 해석한다.
    'data'가 주어지면 이분형(더미) 변수와 연속형 변수를 구분해 해석 표현을 달리한다.

    Args:
        fit: 'fit_model' 함수로 적합된 회귀분석 결과 객체
        data: 원본 데이터 프레임(기본값: None). 주어지면 이분형/연속형을 구분해 표현한다.
...
def plot_odds(fit, data, palette=None, title=None, xlabel=None, ylabel=None,
    ...
    """
    오즈비(Odds Ratio)를 가로 막대 그래프로 시각화해 독립변수의 영향력을 보여준다. 

    막대는 'report_variables'가 정렬해 둔 |B|(=|log OR|) 내림차순 그대로
    위에서 아래로 배치되며, 오즈비가 1보다 큰지(사건 발생 오즈 증가) 작은지(감소)에 따라 
    ...
    Args: 
        ...
        width (int) : 캔버스 가로 픽셀 (기본값: 1280)
        height (int) : 캔버스 세로 픽셀. None이면 독립변수 수 x 80 으로 자동계산 (기본값:None)
        save_path (str) : 이미지 저장 경로 (기본값:None)
    """
    # --- 1) 시각화용 데이터 전처리 ---
    vdf = report_variables(fit, data)
    rdf = vdf[["독립변수", "오즈비(OR)"]].copy()
    # OR>1 이면 증가(+), OR<1이면 감소(-)로 색상 구분 
    rdf["부호"] = np.where(rdf["오즈비(OR)"]>1, "+", "-")

    # 독립변수가 많을 수록 막대가 촘촘해지므로, 변수 하나당 80px씩 세로 공간을 확보한다.
    if height is None: 
        height = len(rdf)*80
...
def test_linear(fit=None, data=None, yname=None, xnames=None, targets=None, alpha=0.05):
    ...
    독립변수 x에 ' x 곱하기 ln(x)' 항을 하나씩 추가해 그 항이 유의한지 본다. 
    ...
    for x in targets:
        ...
        # 최소값이 0이하 이면 ln 정의역(양수)을 맞추기 위해 평행이동 필요 
        shifted = bool(base.min() <= 0)
        if shifted:
            base = base - base.min() + 1

        # Box-Tidwell 항 생성 : x 곱하기 ln(x)
