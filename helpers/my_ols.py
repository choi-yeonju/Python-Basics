...
def predict(fit, new_data): 
    ...
    Returns:
        DataFrame : 새로운 데이터에 대한 예측값을 포함하는 데이터 프레임.
                    컬럼명은 'pred'로 설정된다.
    """
...
def test_linear(fit, alpha=0.05, plot=True, palette=None, title=None, 
    ...
    Ramsey RESET Test(power=2)를 수행하여 적합된 선형모형에 고차항을 추가했을때 
    유의미한 설명력이 남는지를 확인한다. 
...
def test_normal(fit, alpha=0.05, plot=True, palette=None, width=1280, height=640):
    """
    잔차의 정규성을 두가지 방법으로 검정하고 진단 결과를 순서대로 출력한다. 

    Args: 
        fit : 'fit_model' 함수로 적합된 회귀분석 결과 객체 
        alpha (float) : 유의수준 (기본값: 0.05) 
        plot (bool) : Q-Q 플롯과 루트MSE 잔차도를 함께 그릴지 여부 (기본값: True)
    ...
    # --- 4) 루트MSE 구간 규칙(68-95-99.7) 판정 --- 
    # 잔차 표준편차 추정치 루트 MSE를 기준으로 +-1, +-2, +-3 루트MSE 구간의 실제 포함 비율을 구하고,
    # '기대 비율 +-2SE' (SE=루트(p(1-p)/n)) 허용 범위 안에 드는지 확인한다. 
    sqrt_mse = float(np.sqrt(fit.mse_resid))
    expected = [0.68, 0.95, 0.997] # +-1, +-2, +-3 루트MSE 구간의 정규분포 기대 비율
    ...
    print(f"루트MSE = {sqrt_mse:.2f} , 구간 규칙 판정: {'정규성 부합' if mse_rule else '정규성 위배'}") 

    # --- 5) 루트MSE 잔차도 (적합값 대비 잔차 + +-루트MSE 구간)
    if plot: 
        # 팔레트가 지정되면 3색을 뽑아 +-루트MSE 구간 색상으로, 가운데 색을 산점도 색상으로 사용
        ...
        # +-1, +-2, +-3루트MSE 구간 표시 및 포함 비율 주석 
        for i, c in enumerate(band_colors): 
            k = i+1
            y_pos = k*sqrt_mse
            ax.axhline(y=y_pos, color=c, linestyle="--", alpha=0.6)
            ax.axhline(y=-y_pos, color=c, linestyle="--", alpha=0.6)
            ax.text(x=1.02, y=0.5 + 0.12*k, s=f"+{k} 루트MSE = {ratios[i]:.2f}%",
...
def test_independent(fit): 
    ...
    Durbin-Watson 검정은 본래 시계열 데이터 전용이므로, 
    시간 순서가 없는 데이터에서 독립성이 위배되더라도 무시해도 되는 경우가 많다. 
...
def report_variables(fit, data, hc3=False): 
    ...
    완전한 정밀도의 실수값으로 직접 가져온다. 
    표준화 회귀계수(베타)와 공차, VIF 계산에는 원본 데이터의 표준편차가 필요하므로 'data'를 함께 받는다. 

    Args: 
        fit: 'fit_model' 함수로 적합된 회귀분석 결과 객체 
        data: 회귀분석에 사용한 원본 데이터 프레임. 독립변수와 종속변수를 모두 포함해야 한다. 

    Returns: 
        DataFrame: 독립변수별 보고표. 종속변수, 독립변수, B, 표준오차, 베타, t, 유의확률, 공차, VIF 컬럼을 갖는다. 
    ...
    # 독립변수 전체를 대상으로 VIF를 한번에 계산 (상수항 제외한 결과가 반환된다.)
    ...
    # -- (신규) cov_type 지정시 로버스트 표준오차, t, 유의확률을 별도로 계산 
    # 일반값을 덮어쓰지않고 비교용으로 따로 보관한다. 
    # t는 정의상 계수/표준오차 이므로 표준오차가 로버스트로 바뀌면 t도 함께 바뀐다( t= B/로버스트 SE)
    # 유의확률도 이 로버스트 t에서 나온다.
    ...
    for x in xnames: 
        ...
        if hc3 : 
            # 로버스트 비교 형식 
            # : B(양쪽 공유) + 일반(SE, t, 유의확률) + 로버스트(SE, t, 유의확률)를 대칭으로 배치한다.
    ...
    vdf = vdf.sort_values("베타", key=abs, ascending=False).reset_index(drop=True)
    return vdf
...
def plot_beta(fit, data, palette=None, title=None, xlabel=None, ylabel=None, 
    ...
    """표준화 회귀계수(베타)를 가로 막대 그래프로 시각화해 독립변수의 영향력 순위를 보여준다."""

    # --- 1) 시각화용 데이터 전처리 ---
    # 회귀계수 표 리턴 받기 - 베타값 자체는 hc3 여부와 무관하므로 hc3 = False 로 호출한다.
    vdf = report_variables(fit, data, hc3=False)
    rdf = vdf[["독립변수", "베타"]].copy()
    rdf["부호"] = np.where(rdf["베타"] > 0, "+", "-")   # 계수 부호(색상 구분용)

    # 독립변수가 많은 수록 막대가 촘촘해지므로, 변수 하나당 80px씩 세로 공간을 확보한다. 
    if height is None : 
        height = len(rdf) * 80
    ...
    # --- 2) 그래프 초기화 ---
    fig, ax = my_plot.init(width=width, height=height, title=title, 
                           xlabel=xlabel if xlabel else "표준화 계수(베타)",
                           ylabel=ylabel if ylabel else "독립변수")

    # --- 3) 가로 막대 그래프 (값 축을 x로 두면 가로형이 된다.) ---
    my_plot.barplot(rdf, x="베타", y="독립변수", hue="부호", palette=palette, ax=ax)

    # --- 4) 막대 끝에 베타 값 표기 ---
    # 양수 막대는 오른쪽 끝에 바깥쪽 (ha="left"), 음수 막대는 왼쪽 끝의 바깥쪽(ha="right")에 붙도록 정렬 기준을 뒤집고, 
    # 막대와 겹치지 않게 부호 방향으로 살짝 띄운다. 
    for i in rdf.index:
        beta = rdf.loc[i, "베타"]
...
def fit_pipeline(data, y, nominal_cols=None, *  , 
    ...
    Returns :
        적합이 완료된 회귀분석 결과 객체. 보고에 필요한 정보가 아래 속성으로 함께 붙는다. 
            - 'log1p_y'  (bool) / 'log1p_x' (list) : 로그 변환 정보 
            - 'reflect_y' (bool) / 'reflect_x' (list) : 반사 후 로그 변환 정보 
            - 'reflect_y_max_' (float) : 종속변수를 반사할때 쓴 최댓값(역변환용)
            - 'data_' (DataFrame) : 전처리가 끝난 데이터 (베타, VIF 계산용)
    ...
    # 종속변수를 반사할 경우 원 척도로 되돌리려면 변환 당시의 최댓값이 반드시 필요하다.
    reflect_y = False
    reflect_x = []
    reflect_y_max = None 
    ...
    if log: 
        # 8-1) 변환 후보 추리기 
        # 연속형 독립변수와 종속변수가 후보다 (실제로 무엇을 변환할지 통계량이 정한다.)
        ...
        # 우측 꼬리를 값의 위치에 따라 log와 log1p로 갈리고, 좌측 꼬리는 반사 후 log1p로 구분
        log_columns = []
        log1p_columns = []
        reflect_columns = []
        ...
        # 8-3) 변환 실행 (종속변수를 반사하면 역변환용 최댓값을 먼저 남겨둔다)
    ...
    if remain:
        raise ValueError(f'숫자로 변환되지 않은 컬럼이 남아있습니다.: {remain}\n'
                         f'labeling=True 또는 encode=True 로 설정하세요.')
    # 13-2) 결측치 확인 (남아 있으면 OLS가 MissingDataError를 낸다)
    na_cols = df.columns[df.isna().any()].tolist()

    if na_cols:
        raise ValueError(f'결측치가 있는 컬럼이 잇습니다: {na_cols}\n'
    ...
    # 전처리가 끝난 데이터. report_variables(), plot_beta()가 베타, VIF 계산에 사용해야 한다. 
    fit.data_ = df

    # 모델을 구분할 이름( compare_models 가 딕셔너리 키로 채워주기도 한다.)
    fit.name_ = name 
...
def compare_models(fits, metric='RMSE', sub_metric='변수수', tolerance=0.05, 
    ...
    주 지표 1위와의 격차가 tolerance이내면 '근소 격차 그룹'으로 묶어 보조 지표로 순서를 정한다.
    종속변수를 log1p, 반사 변환한 모델은 예측값을 원본 척도로 되돌려 RMSE, MAE, R^2를 계산한다.

    Args:
        fits (dict) : {모델 이름: 적합된 회귀분석 결과 객체 }형태의 딕셔너리 
    ...
    # (모델에 들어간 값 그대로 계산 되므로 로그 변환 여부가 다르면 값의 단위 자체가 다르다 )
    ...
    if len(set(log_flags))>1 and metric in scale_sensitive:
        ...
        print(f'종속변수의 척도가 서로 다른 모델이 섞여있습니다'
              f'(로그, 반사 변환 적용: {applied}개 / 미적용: {len(log_flags)-applied}개).\n'
              f"    '{metric}' 지표는 같은 척도 끼리만 비교할 수 있습니다. "
    ...
    # 각 모델에 딕셔너리 키를 이름으로 새겨둔다. (직접 지정한 name_이 없을때만)
...
def report_model(fit, title=True, plot=True):
    ...
    heading("#### 3) 영향력 순위 시각화 (표준화 계수 베타)")
    plot_beta(fit, data, title="최종 모델의 표준화 회귀계수(베타)-영향력 순위")
    ...
    heading("#### 1) 선형성 검정")
    test_linear(fit, plot=plot, title="적합값 대비 잔차(lowess추세선)")
