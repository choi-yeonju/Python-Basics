...
def report_split(data, column=None, size=3):
    ...
    """
    if column is not None: # data가 데이터 프레임이면 대상 컬럼만 추출
        data = data[column]
...
def report_variation(data, column=None, freq="YE"):
    ...
    Args: 
        ...
        freq (str): 묶을 기간 단위 (기본값: "YE")
            YE -> 연말 기준 1년씩 / QE -> 분기씩 / ME -> 월말 기준 한달 씩, 
    ...
    # groupby()는 "값이 같은 것끼리" 묶는데, 날짜는 값이 달라서 그대로 묶이지 않는다.
...
def adf_test(data, column=None, name=None, alpha=0.05):
    ...
    # --- 2) 결과 구성 및 반환 --- 
    stationary = bool(pvalue < alpha) # pvalue가 작아야 정상성 확보 
...
def adf_diff(data, column=None, max_diff=-1, alpha=0.05):
    ...
    while True: 
        ...
        current_std = row["표준편차"].iloc[0] # 검정 결과를 리스트에 추가 
    ...
    result_df = concat(result) # 결과 리스트를 데이터 프레임으로 합치기 
    ...
    for position, name in enumerate(result_df.index): # 각 행을 순서대로 순회하며 
        if position == best_at: # 표준편차가 최소인 차수하면 
    ...
    # 권장 차수가 아직 비정상이면 차분을 더 하는 것이 아님을 알린다. 
    # 남은 원인은 대게 계절성과 분산이며, 그것은 차분 횟수로 풀 문제가 아니다. 
...
def adf_transform(data, column=None, period=None, max_diff=2, log=True, alpha=0.05):
    ...
    Args: 
        data (DataFrame | Series): 검정할 시계열 데이터
        column (str): data가 데이터프레임인 경우 대상 컬럼명 (기본값: None)
        period (int): 계절주기. None이면 계절차분 조합을 만들지 않음 (기본값: None)
    ...
    for use_log in log_options: # 로그변환 여부 후보 순회 
        for diff in range(max_diff + 1): # 일반차분 횟수 후보 순회 
            for seasonal_diff in seasonal_options: # 계절차분 여부 후보 순회 
                ...
                # --- 2-1) 조합 이름 구성 ---
                parts = []
                if use_log: # 로그변환을 적용했다면 조합 이름에 추가 
                    parts.append("로그변환")
                ...
                target.name = name # 변환결과에 조합 이름을 붙인다 

                changed[name] = target  # 나중에 골라 돌려주기 위해 보관 

                # --- 2-3) ADF 검정 수행 후 결과 저장 ---
...
def compare_smoothing(data, sizes, column=None, method="ma", overlay=False, 
    ...
    """창 크기(또는 span)를 바궈가며 평활 결과를 비교한다. 
    ...
    Args: 
        ...
        overlay (bool): True이면 한 축에 겹쳐 그리고, False면 격자에 나눠 그린다. (기본값: False) 
    ...
    for size in sizes: 
        ...
        if method == "ewm" or method == "both":
            # ewm()의 첫 인자는 span이 아니라 com 이므로 span을 키워드로 명시한다. 
